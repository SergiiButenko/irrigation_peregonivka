...
void setup() {
  ...
  String postStr = host;
  postStr += "/weather_station?api_key=";
  postStr += api_key;
  ...
  postStr += "&measured_humi=";
  postStr += String(measured_humi);
  postStr += "&volt=";
  postStr += String(volt);
  postStr += "&measured_pres=";
  postStr += String(measured_pres);
  postStr += "&DewpointTemperature=";
  postStr += String(DewpointTemperature);
  postStr += "&HeatIndex=";
  postStr += String(HeatIndex);
  postStr += "&status=";
  String w_status = String(forecast_in_words + ": " + ZambrettisWords + ". " + pressure_in_words + " " + trend_in_words + ". " + accuracy_in_words + " " + accuracy_in_percent + "%25.");
  w_status.replace(" ", "%20");
