...
def send_response(viber_request):
    ...
    if text == "тест3" or text == "test3":
        ...
        # r = '{"Type": "keyboard", "Buttons": [{ "Columns": 3, "Rows": 2, "Text": "<font color=\\"#494E67\\">Відмінити цей полив</font><br><br>", "TextSize": "medium", "TextHAlign": "center", "TextVAlign": "bottom", "ActionType": "reply", "ActionBody": "Відмінити", "BgColor": "#f7bb3f", "Image": "https://s12.postimg.org/ti4alty19/smoke.png" }, { "Columns": 3, "Rows": 2, "Text": "<font color=\\"#494E67\\">Non Smoking</font><br><br>", "TextSize": "medium", "TextHAlign": "center", "Tex tVAlign": "bottom", "ActionType": "reply", "ActionBody": "Non smoking", "BgColor": "#f6f7f9", "Image": "https://s14.postimg.org/us7t38az5/Nonsmoke.png"}]}'
        r = '{"Type": "keyboard", "Buttons": [{ "Columns": 3, "Rows": 2, "Text": "<font color=\\"#494E67\\">Відмінити цей полив</font><br><br>", "TextSize": "medium", "TextHAlign": "center", "TextVAlign": "bottom", "ActionType": "reply", "ActionBody": "Відмінити", "BgColor": "#f7bb3f"}, { "Columns": 3, "Rows": 2, "Text": "<font color=\\"#494E67\\">Відмінити всі поливи на сьогодні</font><br><br>", "TextSize": "medium", "TextHAlign": "center", "Tex tVAlign": "bottom", "ActionType": "reply", "ActionBody": "Відмінити", "BgColor": "#f6f7f9"}]}'

        viber.send_messages(
            sender_id, [TextMessage(text="Все ок", keyboard=json.loads(r))]
        )
    ...
    if text.startswith("відмінити"):
        ...
        if not res:
            viber.send_messages(
                sender_id, [TextMessage(text="Перевірте чи Ви все правильно надіслали")]
            )
            return
        ...
        try:
            ...
            response_status = requests.get(
                url="http://mozart.hopto.org:7542/cancel_rule",
                params=payload,
                timeout=(10, 10),
            )
            ...
            viber.send_messages(
                ...
                [
                    TextMessage(
                        text="Не вдалося відмінити правило. Передіть на сайт або спробуйте ще раз."
                    )
                ],
            )
        else:
            for user in USERS:
                ...
                logger.info(
                    "Sending message to {0}. id: {1}".format(user["name"], user["id"])
                )
                viber.send_messages(
                    user["id"],
                    [
                        TextMessage(
                            text="Користувач {0} відмінив цей полив".format(sender_name)
                        )
                    ],
                )
...
def notify_users():
    logger.debug(
        "received request for send_message. post data: {0}".format(request.get_data())
    )
    ...
    for user in users:
        ...
        viber.send_messages(
            ...
            [
                ...
                TextMessage(
                    text="Через {0} хвилин почнеться полив гілки '{1}'. Триватиме {2} хвилин.\nДля того, щоб відмнінити цей полив, відправте мені повідомлення \n'Відмінити {3}'".format(
                        timeout, user_friendly_name, time, rule_id
                    )
                )
...
def send_message():
    ...
    for user in users:
        logger.info("Sending message to {0}. id: {1}".format(user["name"], user["id"]))
        viber.send_messages(user["id"], [TextMessage(text=msg_text)])
