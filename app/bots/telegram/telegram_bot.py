...
def notify_filled():
    logging.info(
        "received request for notify_filled. post data: {0}".format(request.get_data())
    )
...
def send_message():
    ...
    for user in users:
        logging.info(
            "Sending message '{0}'' to {1}. id: {2}".format(
                str(message), user["name"], user["id"]
            )
        )
...
def notify_users():
    ...
    for user in users:
        ...
        res = bot.send_message(
            user["id"],
            "Через {0} хвилин почнеться полив гілки '{1}'. Триватиме {2} хвилин.".format(
                timeout, user_friendly_name, time
            ),
        )
        logging.debug("Responce: {0}".format(str(res)))
