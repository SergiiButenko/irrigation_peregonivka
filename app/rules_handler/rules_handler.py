...
def send_to_viber_bot(rule):
    ...
    if delta_min <= 1:
        ...
        message = "Через {0} хвилин почнеться полив гілки '{1}'. Триватиме {2} хвилин.".format(
            VIBER_SENT_TIMEOUT, user_friendly_name, time
        )
    ...
    try:
        ...
        logging.debug(
            "REDIS_KEY_FOR_VIBER: {0} expires in 12 hours".format(REDIS_KEY_FOR_VIBER)
        )
    ...
    try:
        ...
        while True:
            ...
            for rule in RULES_FOR_BRANCHES:
                ...
                if now_time >= (
                    rule["timer"] - datetime.timedelta(minutes=VIBER_SENT_TIMEOUT)
                ):
                    try:
                ...
                if now_time >= rule["timer"]:
                    ...
                    logging.info(
                        "Rule '{0}' execution is about to start. Checking time delta not more than '{1}' minutes".format(
                            str(rule), MAX_DELTA_FOR_RULES_SERVICE
                        )
                    )
